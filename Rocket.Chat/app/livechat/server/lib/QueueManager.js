...
export const queueInquiry = async (room, inquiry, defaultAgent, jwtToken, name1, email) => {
	...
	if (inquiry.status === 'ready') {
		...
		return RoutingManager.delegateInquiry(inquiry, inquiryAgent, jwtToken, name1, email);
	}
};
...
export const QueueManager = {
	async requestRoom({ guest, message, roomInfo, agent, extraData }) {
		...
		const result1 = LivechatVisitors.findOneById(guest._id);
		//const jwtToken = result1.livechatData.jwt_token;
		var jwtToken = '';
		//console.log(result1);
		...
		//console.log(name1);
		const email = result1.visitorEmails[0].address;
		//console.log(email);	
		if(result1.hasOwnProperty('livechatData')){
			jwtToken = result1.livechatData.jwt_token;
		...
		await queueInquiry(room, inquiry, agent, jwtToken, name1, email);
