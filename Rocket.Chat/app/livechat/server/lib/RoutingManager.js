...
export const RoutingManager = {
	...
	async getNextAgent(department, jwtToken, name1, email, ignoreAgentId) {
		logger.debug(`Getting next available agent with method ${ this.methodName }`);
		return this.getMethod().getNextAgent(department, jwtToken, name1, email, ignoreAgentId);
	},

	async delegateInquiry(inquiry, agent, jwtToken, name1, email, options = {}) {
		...
		if (!agent || (agent.username && !Users.findOneOnlineAgentByUserList(agent.username) && !allowAgentSkipQueue(agent))) {
			...
			agent = await this.getNextAgent(department, jwtToken, name1, email);
