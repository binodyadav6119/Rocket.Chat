...
export class LivechatAgentActivityRaw extends BaseRaw<ILivechatAgentActivity> {
	findAllAverageAvailableServiceTime({ date, departmentId , type}: { date: Date; departmentId: string , type: string}): Promise<ILivechatAgentActivity[]> 
	{
		...
		if (type == "department") {
			...
			departmentsMatch = {
				$match: {
					'departments.agentId': departmentId,
				},
			};
