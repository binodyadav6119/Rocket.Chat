...
export class LivechatRoomsRaw extends BaseRaw {
	...
	async findAllNumberOfAbandonedRooms({
		...
		if (type == "department") {
			...
			match.$match['servedBy._id'] = departmentId;
		}
		const sort = {
			$sort: options.sort || {
				name: 1
	...
	findAllAverageWaitingTime({
		...
		if (type == "department") {
			match.$match.departmentId = departmentId;
			console.log("department waiting livechat room");
		} else {
			match.$match['servedBy._id'] = departmentId;
			console.log("agent waiting livechat room");
		}
		const sort = {
			$sort: options.sort || {
				name: 1
	...
	getOnHoldConversationsBetweenDate(from, to, departmentId, type) {
		const query = {
			onHold: {
				$exists: true,
				$eq: true,
		...
		if (type == "department") {
			...
			var value = ['servedBy._id'];
			query.value = departmentId;
		}

		// console.log("inside getOnHoldconversationBw Date");
