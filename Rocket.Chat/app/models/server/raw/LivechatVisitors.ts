...
export class LivechatVisitorsRaw extends BaseRaw<ILivechatVisitor> {
	getVisitorsBetweenDate({ start, end, department, type }: { start: Date; end: Date; department: string, type: string }): Cursor<ILivechatVisitor> {
		if (type == "department") {
			console.log("getVisitorsBetweenDate");
			const query = {
				_updatedAt: {
					$gte: new Date(start),
					$lt: new Date(end),
